...
interface GroupsData {
    ...
    function getNodesInGroup() external view returns (uint[]);
...
contract SkaleDKG is Permissions {
    ...
    function broadcast(bytes32 groupIndex, uint nodeIndex, bytes verificationVector, bytes secretKeyContribution) public {
    ...
    function adding(bytes32 groupIndex, uint x1, uint y1, uint x2, uint y2, uint x3, uint y3) internal {
        if (channels[groupIndex].publicKeyx.x == 0 && channels[groupIndex].publicKeyx.y == 0 && channels[groupIndex].publicKeyy.x == 0 && channels[groupIndex].publicKeyy.y == 0) {
            ...
            addG2(groupIndex, a, b, channels[groupIndex].publicKeyx, channels[groupIndex].publicKeyy);
        }
    }
    ...
    function addFp2(Fp2 a, Fp2 b) internal view returns (Fp2) {
        return Fp2({ x: addmod(a.x, b.x, p), y: addmod(a.y, b.y, p) });
    }

    function minusFp2(Fp2 a, Fp2 b) internal view returns (Fp2) {
        uint first;
        uint second;
        if (a.x >= b.x) {
    ...
    function mulFp2(Fp2 a, Fp2 b) internal view returns (Fp2) {
    ...
    function squaredFp2(Fp2 a) internal view returns (Fp2) {
    ...
    function doubleG2(Fp2 x1, Fp2 y1, Fp2 z1) internal view returns (Fp2, Fp2) {
    ...
    function U1(Fp2 x1) internal view returns (Fp2) {
        return mulFp2(x1, squaredFp2(Fp2({ x: 1, y: 0 })));
    }

    function U2(Fp2 x2) internal view returns (Fp2) {
        return mulFp2(x2, squaredFp2(Fp2({ x: 1, y: 0 })));
    }

    function S1(Fp2 y1) internal view returns (Fp2) {
        return mulFp2(y1, mulFp2(Fp2({ x: 1, y: 0 }), squaredFp2(Fp2({ x: 1, y: 0 }))));
    }

    function S2(Fp2 y2) internal view returns (Fp2) {
        return mulFp2(y2, mulFp2(Fp2({ x: 1, y: 0 }), squaredFp2(Fp2({ x: 1, y: 0 }))));
    }

    function isEqual(Fp2 u1, Fp2 u2, Fp2 s1, Fp2 s2) internal pure returns (bool) {
        return (u1.x == u2.x && u1.y == u2.y && s1.x == s2.x && s1.y == s2.y);
    }

    function zForAddingG2(Fp2 u2, Fp2 u1) internal view returns (Fp2) {
    ...
    function yForAddingG2(Fp2 s2, Fp2 s1, Fp2 u2, Fp2 u1, Fp2 x) internal view returns (Fp2) {
    ...
    function xForAddingG2(Fp2 s2, Fp2 s1, Fp2 u2, Fp2 u1) internal view returns (Fp2) {
        Fp2 memory r = addFp2(minusFp2(s2, s1), minusFp2(s2, s1));
        Fp2 memory I = squaredFp2(addFp2(minusFp2(u2, u1), minusFp2(u2, u1)));
        Fp2 memory V = mulFp2(u1, I);
        Fp2 memory J = mulFp2(minusFp2(u2, u1), I);
    ...
    function addG2(bytes32 groupIndex, Fp2 x1, Fp2 y1, Fp2 x2, Fp2 y2) internal {
    ...
    function inverseFp2(Fp2 a) internal view returns (Fp2 x) {
    ...
    function toAffineCoordinatesG2(Fp2 x1, Fp2 y1, Fp2 z1) internal view returns (Fp2 x, Fp2 y) {
        if (z1.x == 0 && z1.y == 0) {
            x.x = 0;
            x.y = 0;
            y.x = 1;
