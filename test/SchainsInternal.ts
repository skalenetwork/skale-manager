...
describe("SchainsInternal", () => {
    ...
    it("should set generation to schain", async () => {
        ...
        await schainsInternal.initializeSchain(generation0Name, holder.address, ethers.constants.AddressZero, 5, 5, "0x");
        (await schainsInternal.getGeneration(generation0Hash)).should.be.equal(generation);

        await schainsInternal.newGeneration();
        generation = generation.add(1);

        await schainsInternal.initializeSchain(generation1Name, holder.address, ethers.constants.AddressZero, 5, 5, "0x");
        ...
        describe("on registered schain", async () => {
            ...
            fastBeforeEach(async () => {
                ...
                for (const schainName of newSchainNames) {
                    await schainsInternal.initializeSchain(schainName, owner.address, ethers.constants.AddressZero, 5, 5, "0x");
                }
            });
