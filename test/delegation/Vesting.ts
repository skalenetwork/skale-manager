...
contract("Vesting", ([owner, holder, holder1, holder2, holder3, hacker]) => {
    ...
    it("should register SAFT investor", async () => {
        ...
        await Vesting.connectHolderToSAFT(holder, 0, getTimeAtDate(1, 6, 2020), 1e6, 1e5, {from: owner});
    ...
    it("should not connect holder to SAFT if sender is not owner", async () => {
        await Vesting.addSAFTRound(6, 36,  6, false, {from: owner});
        await Vesting.connectHolderToSAFT(holder, 0, getTimeAtDate(1, 6, 2020), 1e6, 1e5, {from: hacker}).should.be.eventually.rejectedWith("Ownable: caller is not the owner");
        // await Vesting.addVestingTerm(holder, getTimeAtDate(1, 6, 2020), 6, 36, 1e6, 1e5, 6, false, {from: hacker}).should.be.eventually.rejectedWith("Ownable: caller is not the owner");
    });
    ...
    it("should not connect holder to SAFT Round if amounts incorrect", async () => {
        await Vesting.addSAFTRound(6, 36,  6, false, {from: owner});
        await Vesting.connectHolderToSAFT(holder, 0, getTimeAtDate(1, 6, 2020), 1e5, 1e6, {from: owner}).should.be.eventually.rejectedWith("Incorrect amounts");
    });
    ...
    //         // await Vesting.addVestingTerm(holder, startDate, lockupPeriod, fullPeriod, fullAmount, lockupAmount, vestPeriod, isCancelable, {from: owner});
    //         await Vesting.addSAFTRound(lockupPeriod, fullPeriod,  vestPeriod, isCancelable, {from: owner});
    //         await Vesting.connectHolderToSAFT(holder, 0, startDate, fullAmount, lockupAmount, {from: owner});
    //         await Vesting.approveSAFTHolder({from: holder});
    //         await Vesting.startVesting(holder, {from: owner});
    //         // SAFT example 1
    ...
    //         await Vesting.connectHolderToSAFT(holder1, 1, startDate, fullAmount1, lockupAmount1, {from: owner});
    ...
    //         await Vesting.connectHolderToSAFT(holder2, 2, startDate, fullAmount2, lockupAmount2, {from: owner});
    ...
    //         await Vesting.connectHolderToSAFT(holder3, 3, startDate, fullAmount3, lockupAmount3, {from: owner});
