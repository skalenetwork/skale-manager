...
contract("TokenSaleManager", ([owner, holder, delegation, validator, seller, hacker]) => {
    ...
    beforeEach(async () => {
        ...
        tokenSaleManager = await TokenSaleManager.new();
    ...
    it("should not register seller if sender is not owner", async () => {
        await tokenSaleManager.registerSeller(seller, {from: hacker}).should.be.eventually.rejectedWith("Only owner can register sellers");
    });
    ...
    describe("when seller is registered", async () => {
        ...
        it("should allow seller to approve transfer to buyer", async () => {
            ...
            await skaleToken.balanceOf(holder).should.be.eventually.equal("10");
        });
