...
contract("SkaleDKG", ([owner, validator, developer, hacker]) => {
    ...
    describe("when 2 nodes are created", async () => {
        ...
        it ("should encrypt and decrypt messages with a given key correctly", async() => {
            ...
            console.log(new BigNumber(decrypted.toString()));
            console.log(new BigNumber(decrypted.toString()).toString() == secretNumber.toString());
            assert(new BigNumber(decrypted.toString()).toString() == secretNumber.toString());
        });
